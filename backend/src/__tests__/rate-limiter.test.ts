...
describe('Rate Limiter', () => {
  ...
  describe('Multiple Users', () => {
    ...
    it('should track usage independently for multiple users', () => {
      ...
      expect(status1.tokensRemaining).toBe(RATE_LIMIT_CONFIG.BUCKET_CAPACITY - 1)
      expect(status2.tokensRemaining).toBe(RATE_LIMIT_CONFIG.BUCKET_CAPACITY - 2)
      expect(status3.tokensRemaining).toBe(RATE_LIMIT_CONFIG.BUCKET_CAPACITY - 3)
    })
  })
  ...
  describe('Rate Limit Status', () => {
    ...
    it('should return correct status after consuming tokens', () => {
      ...
      expect(status.tokensRemaining).toBe(RATE_LIMIT_CONFIG.BUCKET_CAPACITY - tokensConsumed)
    })
    ...
    it('should calculate next refill time correctly', () => {
      ...
      const status = getRateLimitStatus(userId)
      expect(status.nextRefillIn).toBe(RATE_LIMIT_CONFIG.REFILL_INTERVAL_MS - partialTime)
    })
  })
  ...
  describe('Rate Limit Statistics', () => {
    ...
    it('should return correct stats with active users', () => {
      ...
      expect(stats.totalTokensInCirculation).toBe(3 * (RATE_LIMIT_CONFIG.BUCKET_CAPACITY - 1))
    })
  })
